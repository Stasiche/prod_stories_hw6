...
class LevenshteinPreSuggester(BasePreSuggester):
    ...
    def get_suggestions(self, misspelling: str) -> np.ndarray:
        ...
        for word in self.dict.dic.words:
            if word.stem.isnumeric() or len(word.stem) < 3:
                continue
            dist = -lev.distance(misspelling, word.stem)
            if dist <= self.th:
                if len(suggestions) < self.max_suggestions:
                    heappush(suggestions, (dist, word.stem))
                else:
                    heappushpop(suggestions, (dist, word.stem))
